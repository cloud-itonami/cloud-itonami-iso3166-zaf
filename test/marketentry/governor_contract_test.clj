...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Kita Systems ZA" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest za-presence-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:za-presence-missing} (-> (store/ledger db) last :basis)))
...
(deftest cipc-unverified-is-held-and-unoverridable
  (testing "unverified corporate number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:cipc-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
