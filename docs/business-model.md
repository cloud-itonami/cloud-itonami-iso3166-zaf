# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — South Africa

## Classification

- Repository: `cloud-itonami-iso3166-zaf`
- ISO 3166: `ZAF` (South Africa)
...
- Social impact: [:b-bbee-scorecard-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a South African
  public contract
...
- registration walkthrough for the Central Supplier Database (CSD) and National Treasury's eTender Publication Portal
- business/tax registration checklist: a Companies and Intellectual Property Commission (CIPC) company registration plus CSD supplier registration; VAT registration with the South African Revenue Service (SARS) where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: B-BBEE (Broad-Based Black Economic Empowerment) scorecard status, which materially weights public-tender evaluation and is a mandatory disclosure for most government contracts
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to South African-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in South Africa).
