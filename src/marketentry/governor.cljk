(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a South Africa-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. South Africa-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-za-presence?
                                       true` (near-universal for South Africa
                                       ...
                                       Grounded in South African CSD
                                       registration typically required for
                                       public-sector awards (National Treasury
                                       / organs of state).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CIPC/統一資格/eTender/CSD登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- za-presence-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-za-presence? true`, INDEPENDENTLY verify
  `:has-za-presence?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-za-presence?` ground truth (most South Africa public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-za-presence? e))
                 (not (true? (:has-za-presence? e))))
        [{:rule :za-presence-missing
          :detail (str subject " は南アフリカ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cipc-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cipc? true`, INDEPENDENTLY check
  `:cipc-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cipc? e))
                 (not (true? (:cipc-verified? e))))
        [{:rule :cipc-unverified
          :detail (str subject " はCIPC確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (za-presence-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (cipc-unverified-violations request st)
