(ns marketentry.marketentryllm
  ...
  the ZAF public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-za-presence?=" (:has-za-presence? e)
                        " cipc-verified?=" (:cipc-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-za-presence? e))
                              (:has-za-presence? e))
                          (or (not (:requires-cipc? e))
                              (:cipc-verified? e)))
                   0.9 0.3)}))
